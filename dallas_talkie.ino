// Talkie and dallas one wire temperature sketch
// Puts board in sleep mode in specified period in order to save power

/* Connections:
 
 #### Dallas One Wire Temp sensor #####
 Don't pay attention on the wires :), the colors are specified just for my
 project
 
 Dallas 1-st pin - Power ground       // dark blue
 Dallas 2-nd pin - Digital pin 2      // green
 Dallas 3-rd pin - Power 5V           // red
 
 #### Speaker pins #####
 
 Speaker 1-st pin - Digital ground    // yellow
 Speaker 2-nd pin - Digital pin 3     // blue
...
// blink led when measuring temperature
#define LED_PIN (13)

// Data wire is plugged into pin 2 on the Arduino
#define ONE_WIRE_BUS (2)

 //how long you want the arduino to sleep
#define SLEEP_MSECS (100000L)
...
uint8_t const spELEVEN[]   PROGMEM = {
...
uint8_t const spTWELVE[]   PROGMEM = {
...
uint8_t const spTWENTY[]   PROGMEM = {
...
uint8_t const spHUNDRED[]  PROGMEM = {
...
uint8_t const spTHOUSAND[] PROGMEM = {
...
void sayFloat(float n) {

  long n1 = n;
  float d = n - n1;
  long n2 = d * 100; // use last 2 digits
...
void setup() {
  Serial.begin(9600);
  Serial.println("Dallas Temperature IC Control Library with talkie");

  // Start up the temperature measuring library
  sensors.begin();
...
void loop(void)
{
  ...
  sayFloat(temperature);

  digitalWrite(LED_PIN, LOW);
  
  sleep.pwrDownMode(); //set sleep mode
  sleep.sleepDelay(SLEEP_MSECS);
}
